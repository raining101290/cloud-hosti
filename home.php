                    ...
                    if (data.status === "available") {
                        resultBox.innerHTML = `
                             <div class='alert alert-success' role='alert'><p class="text-success mb-0 d-flex justify-content-between align-items-center" style="font-weight:bold;">
                                ✔ ${domain} is available <button class="btn btn-sm btn-success"
        onclick="addDomainToCart('${domain}')">Add to Cart</button>
                            </p></div>
                        `;
                    }
    ...
    <script>
        ...
        function addDomainToCart(domain) {
            ...
            fetch("includes/apis/cart/get-cart-raw.php")
                ...
                .then(data => {
                    ...
                    // Restrict to 1 domain only
                    const hasDomain = cart.some(item => item.type === 'domain');

                    if (hasDomain) {
                        alert("You already added a domain. Only 1 domain allowed at a time.");
                        return;
                    }

                    // Add domain to backend cart
                    fetch("includes/apis/cart/add-domain.php", {
                        method: "POST",
        ...
        function processCheckout() {
            fetch("includes/apis/cart/get-cart-raw.php")
                .then(res => res.json())
                .then(data => {
                    // console.log('JSON.stringify(data.cart)', JSON.stringify(data.cart))
                    const encodedCart = btoa(JSON.stringify(data.cart));
                    window.location.href ="https://portal.cloudhosti.com/cart-bridge.php?data=" + encodedCart;
                });
        }
